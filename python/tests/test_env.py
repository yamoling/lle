...
def test_available_actions():
    ...
    assert available_actions[1, Action.NOOP.value] == 1
...
def test_available_actions2():
    ...
    def check_available_actions(available: np.ndarray[np.int32], expected_available: list[list[Action]]) -> bool:
        available_actions = np.zeros((2, len(Action)), dtype=np.int32)
    ...
    assert check_available_actions(
        obs.available_actions, [[Action.SOUTH, Action.WEST, Action.NOOP], [Action.SOUTH, Action.EAST, Action.NOOP]]
    )
    ...
    obs, *_ = env.step([Action.SOUTH.value, Action.SOUTH.value])
    check_available_actions(obs.available_actions, [[Action.NOOP] * 2])
