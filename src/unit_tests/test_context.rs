...
fn reachable_laser_paths_simple_world() {
    ...
    assert_eq!(ctx.get_reachable_laser_path(0, 0).len(), 0);
    assert_eq!(ctx.get_reachable_laser_path(0, 1).len(), 0);
    assert_eq!(ctx.get_reachable_laser_path(0, 2).len(), 1);
    assert!(ctx.get_reachable_laser_path(0, 2).contains(&pos(1, 1)));
...
fn reachable_laser_paths_unblockable() {
    ...
    for t in 0..t_max {
        ctx.update(t);
        assert!(ctx.get_reachable_laser_path(0, t).is_empty(), "t={t}");
    }
}
...
fn reachable_laser_paths_increase_then_decrease_over_time() {
    ...
    for t in 0..13 {
        assert_eq!(ctx.get_reachable_laser_path(0, t).len(), t + 1, "t={t}");
    }
    for i in 0..14 {
        assert_eq!(ctx.get_reachable_laser_path(0, t_max - i).len(), i, "i={i}");
    }
}
