...
src/
  ...
  bindings/           – PyO3 wrappers; one Py* struct per Rust type
    ...
    solver/           – PyClauseGenerator, PySolveMode; registered directly into
                        `sys.modules` as `lle.solver.constraints` (see below)
  action.rs           – Action enum (North/South/East/West/Stay)
  agent.rs            – Agent + AgentId
  position.rs         – Position {i, j, k}, k = floor/layer index, with Add<Action>
  solver/             – Rust-native SAT clause generation for bounded planning
    context.rs        – ConstraintContext: reachability pruning over positions
    position_set.rs   – PositionSet: dense bitset over positions for hot reachability paths
    clauses/          – clause generators: movement, lasers, cooperation, assumptions, objective
  rendering/          – image rendering via the `image` crate
  stub_gen.rs         – binary that writes .pyi files via pyo3-stub-gen
```

**Solver is single-floor:** `src/solver/**` has no notion of floors/layers — `ConstraintContext` and `PositionSet` are indexed purely by `height x width`, and every `Position` they build or destructure uses `k: 0` (`Position::new2d`). If `Position` gains fields or a new dimension in the future, the solver module needs matching updates even though it won't show up as a git conflict (it lives in a different part of the tree than the world/tile code that owns `Position`).
...
python/lle/
  ...
  solver/             – SAT-based solver: `solve()`/`solve_model()` build the SAT model with
                        clauses generated by the Rust `ClauseGenerator` (compiled extension,
                        exposed as `lle.solver.constraints`; see `src/solver/` above), then hand
                        it to pysat/Minisat22 to actually solve
    variable_factory.py – SAT variable id allocation (`agent`, `agent_at_exit`, …)
    types.py          – SolveModeLiteral
